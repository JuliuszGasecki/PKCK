...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="AveragePrice">
        <xsl:element name="averagePrice">
            ...
            <xsl:value-of select="concat(format-number(sum(/GamesCatalog/games/game/premierPrice) div count(/GamesCatalog/games/game/premierPrice), '#.##'), ' ', $currnecy[1])" />
        </xsl:element>
    </xsl:template>
    <xsl:template name="AverageUserRate">
        <xsl:element name="averageUserRate">
            <xsl:value-of select="format-number(sum(/GamesCatalog/games/game/metariticRate/userScore) div count(/GamesCatalog/games/game/metariticRate/userScore), '#.##')" />
        </xsl:element>
    </xsl:template>
    <xsl:template name="AverageCrirticRate">
        <xsl:element name="averageCrirticRate">
            <xsl:value-of select="format-number(sum(/GamesCatalog/games/game/metariticRate/metascore) div count(/GamesCatalog/games/game/metariticRate/metascore), '#.##')" />
        </xsl:element>
    </xsl:template>
